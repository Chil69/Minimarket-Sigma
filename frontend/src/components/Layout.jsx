        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {filtered.map((item, idx) => (
            <Link key={item.label} to={item.href} title={!sidebarOpen ? item.label : undefined}
              className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${isActive(item.href) ? 'sidebar-link-active' : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}
