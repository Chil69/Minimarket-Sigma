...
import { Plus, X, DollarSign, Clock, CheckCircle, AlertCircle, Wallet, History, TrendingUp, RefreshCw, FileText, Loader2 } from 'lucide-react';
import jsPDF from 'jspdf';
import 'jspdf-autotable';
import { cashDrawerService } from '../services/api';
import usePOSStore from '../stores/posStore';

function generateHistoryPDF(historyData) {
  const doc = new jsPDF();
  ...
  doc.text('HISTORIAL DE CAJA', pageWidth / 2, 18, { align: 'center' });
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('MiniMarket POS - Sistema de Punto de Venta', pageWidth / 2, 28, { align: 'center' });
  doc.setFontSize(8);
  doc.text(`Generado: ${fechaGeneracion}`, pageWidth / 2, 40, { align: 'center' });
  ...
  if (historyData && historyData.length > 0) {
    doc.autoTable({
      startY: y,
      head: [['Apertura', 'Cierre', 'Inicial', 'Ventas', 'Balance', 'Estado', 'Nota']],
      body: historyData.map(h => {
        const apertura = h.fechaApertura ? new Date(h.fechaApertura).toLocaleString('es-PE') : '—';
        const cierre = h.fechaCierre ? new Date(h.fechaCierre).toLocaleString('es-PE') : '—';
        const inicial = `S/ ${parseFloat(h.montoInicial || 0).toFixed(2)}`;
        const ventas = `S/ ${parseFloat(h.montoCierre || 0).toFixed(2)}`;
        const balance = `S/ ${(parseFloat(h.montoInicial || 0) + parseFloat(h.montoCierre || 0)).toFixed(2)}`;
        const estado = h.fechaCierre ? 'CERRADA' : 'ABIERTA';
        const nota = h.observaciones || '—';
        return [apertura, cierre, inicial, ventas, balance, estado, nota];
      }),
      theme: 'grid',
    ...
    doc.text('No hay historial de cajas disponible.', 14, y);
  }

  doc.save(`Historial_Cajas_${now.toISOString().slice(0, 10)}.pdf`);
}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {activeTab === 'history' && (
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.02]"
        ...
        <div className="glass-card p-0 overflow-hidden">
          {history.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="table-dark w-full">
                <thead>
                  <tr><th>Apertura</th><th>Cierre</th><th>Inicial</th><th>Ventas</th><th>Balance</th><th>Estado</th><th>Observación</th></tr>
                </thead>
                <tbody>
                  {history.map((h, i) => (
                    <tr key={i}>
                      ...
                      <td className="text-slate-400 text-xs max-w-[150px] truncate" title={h.observaciones || '—'}>{h.observaciones || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
