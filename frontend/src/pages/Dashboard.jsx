          ...
          {alerts.length > 0 ? (
            <div className="space-y-3">
              {alerts.map((a, i) => (
                ...
                >
                  <div>
                    <p className="font-medium text-white text-sm">{a.productName || a.ProductName}</p>
                    <p className="text-xs text-slate-400 mt-0.5">Stock: {a.stockActual ?? a.StockActual} — Mínimo: {a.stockMinimo ?? a.StockMinimo}</p>
                  </div>
                  <span className="badge-rose text-xs px-2.5 py-1 rounded-full font-semibold whitespace-nowrap">
                    -{a.faltanUnidades ?? a.FaltanUnidades} unid.
                  </span>
                </div>
              ))}
            </div>
